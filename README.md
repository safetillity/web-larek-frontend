...
5. Класс OrderPresenter
Класс OrderPresenter представляет собой компонент, который управляет процессом оформления заказа в приложении. Он отвечает за обработку изменений в заказе, связанных с выбором способа оплаты, и взаимодействует с формой оплаты для обновления пользовательского интерфейса.

Поля:
private appData: AppData: Экземпляр класса AppData, который содержит данные о текущем состоянии приложения, включая информацию о заказе. Инициализируется через конструктор.

private paymentForm: PaymentForm: Экземпляр класса PaymentForm, который представляет собой интерфейс для взаимодействия с формой оплаты. Инициализируется через конструктор.

Конструктор:
constructor(appData: AppData, paymentForm: PaymentForm): Конструктор принимает два параметра: appData (экземпляр класса AppData), который содержит данные о заказе, и paymentForm (экземпляр класса PaymentForm), который отвечает за отображение формы оплаты. Внутри конструктора вызывается метод init, который настраивает обработчики событий.
Методы:
private init(): void: Приватный метод, который инициализирует обработчики событий. В данном случае он подписывается на событие order:changed, которое генерируется при изменении заказа. Обработчик событий связывается с методом handleOrderChange.

private handleOrderChange(params: { payment: string; button: HTMLElement }): void: Приватный метод, который обрабатывает изменения в заказе. Он принимает объект с двумя параметрами: payment (строка, представляющая способ оплаты) и button (HTML-элемент кнопки, связанной с выбором способа оплаты). Метод обновляет поле заказа с новым значением способа оплаты и вызывает метод highlightPaymentButton у формы оплаты для визуального выделения выбранной кнопки.
...
updateActionLabel(card: ICard): string — метод, который обновляет текст кнопки действия в зависимости от состояния товара (в корзине или нет).
